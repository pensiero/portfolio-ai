...
"shuffle" that loads one of 31 hand-designed art-style pages on every visit.
...
lib/llm.js           Swappable LLM provider (local Ollama today; cloud later)
...
Requires [Node.js](https://nodejs.org) and a local [Ollama](https://ollama.com).
...
| ------------------ | ---------------------------------------- | ---------------------------------------- |
| `PORT`             | `8787`                                   | HTTP port                                |
| `LLM_PROVIDER`     | `ollama`                                 | Provider selector (cloud swap is a config change) |
